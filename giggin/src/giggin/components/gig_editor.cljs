...
(defn form-group [{:keys [id type value values]}]
  [:div.form__group
...
(defn gig-editor [{:keys [modal values initial-values upsert-gig toggle-modal]}]
  (let [{:keys [title desc price img sold-out]} @values]
